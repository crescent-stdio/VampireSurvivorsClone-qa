...
```text
uv run --locked --extra trainer mlagents-learn config/qa-ppo.yaml --run-id=qa-ppo-eval --resume --inference --seed=1234
```
...
Install the locked environment with `uv sync --locked --extra trainer`. Python is fixed to `3.10.8`; do not substitute an unsupported interpreter for ML-Agents Release 20.
