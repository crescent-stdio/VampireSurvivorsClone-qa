...
FAULT_CASES = (
    ...
    (
        ...
        [
            {"observation": {"observation_id": "obs-u0", "inventory": {"abilities": [{"name": "Fire", "level": 1, "owned": True}]}}},
            {"decision": {"action": "select_upgrade"}, "observation": {"observation_id": "obs-u1", "inventory": {"abilities": [{"name": "Fire", "level": 1, "owned": True}]}}},
        ],
    ...
    (
        "health_bar_desync",
        [{"observation": {"observation_id": "obs-v", "player": {"present": True, "health": 8.0}, "player_view": {"health": 10.0}}}],
...
def test_every_private_fault_requires_its_literal_numeric_relation(
    ...
    assert result.target_relations[0] == {
        "field": finding["field"],
        "comparison": finding["comparison"],
        "expected_value": finding["expected_value"],
        "observed_value": finding["observed_value"],
        "evidence_refs": finding["evidence_refs"],
        "integer_values": fault_id
        in {
            "upgrade_ack_without_effect",
            "chest_collected_without_state_transition",
            "currency_leak_across_restart",
...
def test_invalid_pairs_are_counted_by_status_but_excluded_from_confusion_denominators() -> None:
    ...
    conflict = benchmark.score_pair(
        ...
        benchmark.TraceEvaluation.fault("fault-conflict", fault_id, transitions, [artifact(finding)] * 3),
    )
    ...
    assert [pair.status for pair in invalid] == list(benchmark.INVALID_STATUSES)
...
def test_json_and_korean_markdown_present_the_same_trace_level_metrics() -> None:
    ...
    assert "| TP | 2 |" in markdown
    assert "| FN | 1 |" in markdown
    assert "| FP | 1 |" in markdown
    assert "| TN | 2 |" in markdown
    assert "| Macro detection rate | 75.00% | 2 faults |" in markdown
    assert "| Micro detection rate | 66.67% | 2/3 | 20.77%–93.85% |" in markdown
    assert "| health_ratio_out_of_range | 1 | 1 | 1 | 1 | 50.00% | 1/2 | 9.45%–90.55% |" in markdown
    assert "| relative_position_mismatch | 1 | 0 | 0 | 1 | 100.00% | 1/1 | 20.65%–100.00% |" in markdown
