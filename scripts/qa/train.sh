...
cd "$QA_PROJECT_ROOT"
"$QA_UV_BIN" run --locked --extra trainer mlagents-learn "$QA_PPO_CONFIG" --run-id="$QA_PPO_RUN_ID" --env="$QA_MLAGENTS_PLAYER_BUNDLE" --no-graphics \
  --torch-device="$QA_TORCH_DEVICE" --results-dir="$QA_PPO_RESULTS_DIR" \
  --env-args -qaPreset="$QA_PRESET" \
