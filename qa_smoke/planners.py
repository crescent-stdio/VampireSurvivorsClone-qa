...
If the computed value and the reported value disagree, or a fraction falls outside 0 to 1, or a count is negative, or a part exceeds its whole, that is a QA finding. Set reflection.status=unexpected and encode a numeric candidate_id exactly as numeric:<operator>:<field_path>, where operator is eq, ne, lt, le, gt, or ge and field_path is the exact reported field. Keep the two values in qa_observation, recheck the same fields on later observations, and set reproduction_attempted=true when you do. Use a short stable ordinary candidate_id for non-numeric findings."""
