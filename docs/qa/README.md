...
- `Assets/Scenes/QA/QA Gameplay.unity` is copied from Level 1 and references the QA level. Its `QA Episode` root hosts `QaEpisodeController`, `QaGameplayAgent`, `BehaviorParameters`, and `DecisionRequester`.
...
Build scenes are exactly Main Menu (index 0), Level 1 (index 1), then QA Gameplay (index 2).
...
`evaluate.sh` and `replay.sh` use the fixed `-qaSeed=1234`. Evaluation starts `mlagents-learn` with `--resume --inference --seed=1234` before launching the player. Replay accepts a project-relative trace or an explicit absolute file and validates it before launching the player. Training invokes `mlagents-learn config/qa-ppo.yaml`; the behavior name in that file must remain `QaGameplay`.
